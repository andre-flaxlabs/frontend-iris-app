...
const validationSchema = toTypedSchema(
  zod.object({
    name: zod.string().min(1, { message: 'Name of customer is required' }),
    code: zod
      .string()
      .min(1, { message: 'Code of customer is required' })
      .max(5, { message: 'Must have less than 5 characters' }),
    shopify_store: zod.string(),
    shopify_store_slug: zod.string(),
    google_ads_client_id: zod.string().optional(),
    breakeven_roas: zod.number().optional(),
    is_active: zod.boolean().default(true)
  })
)

const { handleSubmit, errors, values, resetForm, setErrors } = useForm({
  validationSchema
})
...
// Mutation function to register customer
const { mutate: registerCustomer } = useMutation({
  mutationFn: (formData: Record<string, any>) => {
    return api.post('/customers/', formData)
    // return fetch('https://on-remotely-pegasus.ngrok-free.app/customers/', {
    //   method: 'POST',
    //   headers: new Headers({
  ...
  >
    <div class="relative bg-white dark:bg-gray-800 rounded-lg p-8 w-full max-w-2xl">
      <div class="flex justify-between items-start pb-3">
        ...
        <button
          @click="closeModal"
          class="absolute top-4 right-4 text-gray-600 hover:text-gray-900 dark:text-white dark:hover:text-violet-600"
        >
          ✕
        </button>
      </div>
      ...
      <form @submit.prevent="onSubmit">
        <div class="flex space-x-4 mb-4">
          <div class="w-2/3">
            ...
            <input
              ...
              type="text"
              class="w-full rounded-lg border border-gray-200 py-3 px-4 focus:outline-none focus:border-white focus:ring-1 focus:ring-violet-500 dark:bg-gray-900 dark:border-gray-700 dark:text-white dark:placeholder-gray-700"
            />
        ...
        <div class="mb-4">
          ...
          <input
            ...
            type="text"
            class="w-full rounded-lg border border-gray-200 py-3 px-4 focus:outline-none focus:border-white focus:ring-1 focus:ring-violet-500 dark:text-white dark:bg-gray-900 dark:border-gray-700 dark:placeholder-gray-700"
          />
        ...
        <div class="flex justify-end">
          ...
          <button
            type="submit"
            class="bg-violet-600 text-sm text-white px-4 py-2 rounded-md hover:bg-violet-700 dark:hover:bg-violet-700"
          >
            Save
          </button>
        </div>
