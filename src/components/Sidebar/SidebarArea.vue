...
<template>
  <aside
    class="absolute left-0 top-0 z-10 flex h-screen w-80 flex-col overflow-y-hidden bg-slate-900 duration-300 ease-linear dark:bg-boxdark lg:static lg:translate-x-0"
    :class="{
  ...
  >
    ...
    <div class="flex items-center justify-between gap-2 px-6 py-5 lg:py-6">
      ...
      <button class="block lg:hidden dark:text-white" @click="sidebarStore.isSidebarOpen = false">
        <svg
          class="fill-current"
          width="20"
          height="18"
    ...
    <div class="custom-scrollbar flex flex-col overflow-y-auto duration-300 ease-linear">
      ...
      <nav class="mt-5 py-4 px-4 lg:mt-9 lg:px-6">
        <template v-for="menuGroup in menuGroups" :key="menuGroup.name">
          <div>
            <h3
              class="mb-4 ml-4 text-sm font-medium text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-600"
            >
              {{ menuGroup.name }}
            </h3>
