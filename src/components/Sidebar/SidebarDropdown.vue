...
<template>
  <ul class="mt-4 mb-6 flex flex-col gap-3 pl-6">
    <template v-for="(childItem, index) in items" :key="index">
      <li>
        <router-link
          :to="childItem.route"
          @click="handleItemClick(index)"
          class="group relative flex items-center gap-3 rounded-md px-4 font-medium text-white duration-300 ease-in-out hover:text-violet-600"
          :class="{
